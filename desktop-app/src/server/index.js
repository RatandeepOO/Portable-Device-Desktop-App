...
app.delete('/api/teams/:id', async (req, res) => {
  try {
    const { id } = req.params;
    console.log(`[DELETE] Attempting to delete team member with ID: ${id}`);

    // First, check if the record exists to distinguish between "not found" and "blocked by RLS"
    ...
    if (fetchError) {
      console.error(`[DELETE] Fetch check failed for ID ${id}:`, fetchError);
    ...
    console.log(`[DELETE] Found team member: ${existing.name}. Proceeding with deletion...`);

    const { data, error } = await supabase.from('users').delete().eq('id', id).select();
    
    if (error) {
      console.error(`[DELETE] Supabase delete error for ID ${id}:`, error);
      throw error;
    }

    if (!data || data.length === 0) {
      console.error(`[DELETE] Delete returned 0 rows for ID ${id}. This likely means RLS is blocking the action.`);
      throw new Error("Delete failed. This action is likely blocked by Row Level Security (RLS) policies on your Supabase 'users' table.");
    }
    
    console.log(`[DELETE] Successfully deleted team member ${id}`);
    broadcastToAll({ type: 'team:updated', action: 'delete', id });
    res.json({ success: true });
  } catch (error) {
    console.error(`[DELETE] Final error catch for ID ${req.params.id}:`, error);
    res.status(500).json({ error: error.message });
  }
});
...
app.put('/api/alerts/:id', async (req, res) => {
  try {
    ...
    if (updates.is_assigned === false && !updates.team_id) {
      ...
      if (fullAlert) {
        const nearest = await findNearestTeam(fullAlert.alert_lat, fullAlert.alert_lng);
        if (nearest) {
          console.log(`Auto-assigning unassigned alert ${id} to team ${nearest.userId}`);
          await assignAlertToTeam(fullAlert, nearest.userId);
        }
      }
    }
...
async function assignAlertToTeam(alert, teamId) {
  ...
  // Update team status in Redis to unavailable
    ...
    try {
      ...
      if (data.type === 'alert:reassign' || data.type === 'alert:forward') {
        ...
        if (targetTeamId) {
          ...
          console.log(`No target team for forward, searching nearest excluding current team ${teamId}...`);
          const nearestTeam = await findNearestTeam(alert.alert_lat, alert.alert_lng, [teamId]);
          
          if (nearestTeam) {
            console.log(`Found nearest team: ${nearestTeam.userId}, assigning...`);
            await assignAlertToTeam(alert, nearestTeam.userId);
          } else {
            console.log('No available teams found for forward.');
            broadcastToAll({
              type: 'alert:updated',
              data: alert
            });
